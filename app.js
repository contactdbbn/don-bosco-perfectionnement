...
function renderMembers(){
 ...
 const filtered=all.filter(m=>!q||String(m.name||"").toLocaleLowerCase("fr-FR").includes(q));
 ...
 const quotaHtml=`<div class="card actual-attendance-card"><div class="row"><div><h2>Quotas des créneaux</h2><div class="muted">Définissez le quota de chaque créneau. Ce réglage est permanent et indépendant du calendrier.</div></div></div><div class="actual-grid">${[1,2,3].map(slot=>{const name=SLOT_NAMES[slot-1],value=getSlotQuota(slot);return `<div class="actual-slot"><div class="row"><div><strong>${name}</strong><div class="muted">Quota permanent</div></div><output id="quotaValue${slot}" class="actual-value">${value}</output></div><input class="actual-range" type="range" min="0" max="30" step="1" value="${value}" oninput="document.getElementById('quotaValue${slot}').value=this.value" onchange="setQuota(${slot},this.value)" aria-label="Quota du ${esc(name)}"><div class="range-scale"><span>0</span><span>15</span><span>30</span></div></div>`}).join("")}</div><div class="muted actual-help">Quota de 0 à 30 personnes par créneau.</div></div>`;
 const listHtml=`<div class="card members-directory-card"><div class="members-directory-head"><div><p class="eyebrow">RÉPERTOIRE</p><h2>Adhérents</h2><div class="muted">Tous les utilisateurs sont affichés ensemble, par ordre alphabétique.</div></div><button class="primary" onclick="addMember()">+ Ajouter un adhérent</button></div><div class="members-search"><label for="membersSearchInput">Rechercher</label><input id="membersSearchInput" type="search" value="${esc(window.membersSearch||"")}" placeholder="Nom de l’adhérent…" oninput="window.membersSearch=this.value;renderMembers()" autocomplete="off"></div><div class="members-directory-list">${filtered.length?filtered.sort((a,b)=>String(a.name||"").localeCompare(String(b.name||""),"fr",{sensitivity:"base"})).map(memberDirectoryCard).join(""):`<div class="empty">Aucun utilisateur ne correspond à la recherche.</div>`}</div></div>`;
 document.getElementById("membersView").innerHTML=stats+listHtml+quotaHtml+(canAdmin()?renderAdminTools():"");
}
