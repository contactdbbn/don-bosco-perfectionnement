...
function renderMembers(){
 ...
 const filtered=all.filter(m=>(membersSlotFilter==="all"||(membersSlotFilter==="none"&&!m.slot)||String(m.slot||"")===membersSlotFilter)&&(!q||String(m.name||"").toLocaleLowerCase("fr-FR").includes(q)));
 ...
 const listHtml=`<div class="card members-directory-card"><div class="members-directory-head"><div><p class="eyebrow">RÉPERTOIRE</p><h2>Adhérents</h2><div class="muted">Tous les utilisateurs sont affichés ensemble, par ordre alphabétique.</div></div><button class="primary" onclick="addMember()">+ Ajouter un adhérent</button></div><div class="members-directory-filters"><div class="members-search"><label for="membersSearchInput">Rechercher</label><input id="membersSearchInput" type="search" value="${esc(window.membersSearch||"")}" placeholder="Nom de l’adhérent…" oninput="window.membersSearch=this.value;renderMembers()" autocomplete="off"></div><label class="members-slot-filter"><span>Créneau</span><select onchange="setMembersSlotFilter(this.value)"><option value="all" ${membersSlotFilter==="all"?"selected":""}>Tous les créneaux</option><option value="1" ${membersSlotFilter==="1"?"selected":""}>Créneau 1</option><option value="2" ${membersSlotFilter==="2"?"selected":""}>Créneau 2</option><option value="3" ${membersSlotFilter==="3"?"selected":""}>Créneau 3</option><option value="none" ${membersSlotFilter==="none"?"selected":""}>Sans créneau</option></select></label></div><div class="members-directory-list">${filtered.length?filtered.sort((a,b)=>String(a.name||"").localeCompare(String(b.name||""),"fr",{sensitivity:"base"})).map(memberDirectoryCard).join(""):`<div class="empty">Aucun utilisateur ne correspond aux critères.</div>`}</div></div>`;
